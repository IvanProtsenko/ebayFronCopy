...
export default function getCustomStatus(conversation) {
    ...
    if (
      ...
      lastMsg.paymentAndShippingMessageType === 'BUYER_OFFER_MADE_BUYER_MESSAGE' &&
      lastMsg.active
    ) {
    ...
    if (
      // 2. Запрос отклонён
      lastMsg.type === 'PAYMENT_AND_SHIPPING_MESSAGE' &&
      lastMsg.paymentAndShippingMessageType === 'SELLER_REJECTED_OFFER_BUYER_MESSAGE'
    ) {
    ...
    if (
      ...
      messages.find(
        msg =>
          msg.type === 'PAYMENT_AND_SHIPPING_MESSAGE' &&
          msg.paymentAndShippingMessageType === 'TRANSACTION_RESERVED_BUYER_MESSAGE' &&
          msg.active
      )
    ) {
    ...
    if (
      ...
      messages.find(
        msg =>
          msg.type === 'PAYMENT_AND_SHIPPING_MESSAGE' &&
          msg.paymentAndShippingMessageType === 'ITEM_MARKED_AS_RECEIVED_BUYER_MESSAGE'
      )
    ) {
  ...
  function isTimePassed(timeshtamp, days) {
    ...
    const now = new Date();
    const daysLag = Math.ceil(Math.abs(now.getTime() - receivedDate.getTime()) / (1000 * 3600 * 24));
    return daysLag > days;
  }